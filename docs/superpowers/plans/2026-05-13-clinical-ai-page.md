# Clinical AI Page — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `/clinical-ai` — a single scrollable page in `medicluster/frontend` that chains NLP note analysis, critical keyword alerts, risk prediction with cluster insights, medication care plan, and patient history into one unified clinical workflow.

**Architecture:** All state lives in `ClinicalAIPage.jsx` (no new contexts or stores). The page calls existing `apiClient.js` functions in sequence, triggered by typing (debounced), button clicks, and patient ID changes. UI is Tailwind-only, following the `Card`/`Btn`/`Field` helper-component pattern already used in `MLToolsPage.jsx`.

**Tech Stack:** React 18, Tailwind CSS 3, `../api/apiClient.js` (axios), `../utils/exportPdf.js` (jspdf + html2canvas already installed)
...
|--------|------|----------------|
| **Create** | `medicluster/frontend/src/pages/ClinicalAIPage.jsx` | Entire Clinical AI page — all 7 sections |
| **Modify** | `medicluster/frontend/src/App.jsx` | Add `/clinical-ai` route |
| **Modify** | `medicluster/frontend/src/components/Navbar.jsx` | Add "Clinical AI" nav link |

> **No backend changes.** All endpoints used already exist.

---

## Note on Testing

`medicluster/frontend` has no automated test runner (no vitest/jest in `package.json`). Each task ends with a **manual browser verification** step. Run `npm run dev` in `medicluster/frontend` (port 3000) before starting.

```bash
cd "medicluster/frontend" && npm run dev
```

---

## Task 1: Route + Nav Wiring
...
Open `medicluster/frontend/src/App.jsx`. Add the import after the `NearbyHospitalsPage` import line, and add the route inside `<Routes>`:

```jsx
// add after: import NearbyHospitalsPage from "./pages/NearbyHospitalsPage";
import ClinicalAIPage from "./pages/ClinicalAIPage";
```

```jsx
// add inside <Routes>, after the /hospitals route:
...
- [ ] **Step 2: Add "Clinical AI" nav link to Navbar.jsx**

In `medicluster/frontend/src/components/Navbar.jsx`, add `{ to: "/clinical-ai", label: "Clinical AI" }` to the nav array, between `"/ask-ai"` and `"/hospitals"`:
...
- [ ] **Step 3: Create a stub ClinicalAIPage.jsx so the route doesn't 404**

Create `medicluster/frontend/src/pages/ClinicalAIPage.jsx`:
...
Navigate to `http://localhost:3000/clinical-ai`. You should see the "Clinical AI" nav link highlighted and the stub heading visible. No console errors.

- [ ] **Step 5: Commit**

```bash
git add medicluster/frontend/src/App.jsx medicluster/frontend/src/components/Navbar.jsx medicluster/frontend/src/pages/ClinicalAIPage.jsx
git commit -m "feat: add /clinical-ai route and nav link (stub)"
```

---

## Task 2: Page Header — Patient Selector + Cluster Model Picker

**Files:**
- Modify: `medicluster/frontend/src/pages/ClinicalAIPage.jsx`

- [ ] **Step 1: Replace stub with full page scaffold and header**

Replace the entire contents of `ClinicalAIPage.jsx` with the following. This adds local helper components, all state declarations, and the page header section. Later tasks will append new `<Card>` sections inside the `<div className="space-y-6">`.
...
Navigate to `http://localhost:3000/clinical-ai`. Check:
- Header renders with "Clinical AI" title, Patient ID input showing "P-001", Cluster Model dropdown (may say "No models" if backend has none — that's fine).
- Two-column grid shows "Clinical Notes" and "Critical Alerts" cards.
- Typing in notes updates character count and, after 30+ chars, triggers the NLP debounce (check Network tab for POST to `/api/ml/analyze-notes` after ~800ms pause).
- Typing "chest pain" shows a red 🚨 alert instantly.
- File drop zone is visible below the textarea.
- Lower sections (Clinical Summary, Risk Prediction, etc.) all render correctly.
- No console errors.

- [ ] **Step 3: Commit**

```bash
git add medicluster/frontend/src/pages/ClinicalAIPage.jsx
git commit -m "feat: implement Clinical AI page — all 7 panels"
```
...
**No code changes** — this task validates the end-to-end flow with real data.

- [ ] **Step 1: Ensure backend is running**

The medicluster backend must be running on port 3001 (or whatever proxy port Vite is configured for). Check `medicluster/frontend/vite.config.js` for the proxy target. Start the backend if needed:

```bash
# from repo root or medicluster/backend:
cd medicluster/backend && python app.py  # or whatever the start command is
```

- [ ] **Step 2: Test NLP analysis**

In the notes textarea, paste:
```
Patient presents with chest pain and breathlessness. HR 112, BP 160/100.
On Metformin 500mg BD and Amlodipine 5mg OD. Glucose 310 mg/dL.
History of hypertension and Type 2 diabetes.
```

Expected:
- Critical Alerts panel shows 🚨 banners for "chest pain" and "breathlessness" immediately.
- After ~800ms, Clinical Summary card shows extracted entities, ICD codes, trajectory badge, and AI summary.
- Drug interaction check fires for Metformin + Amlodipine.

- [ ] **Step 3: Test risk prediction**

Fill in the vitals form: age=65, blood_pressure=160, heart_rate=112, cholesterol=270, bmi=28, glucose=310.

Click "Run Prediction". Expected:
- Risk tier badge appears (HIGH or CRITICAL for these values).
- Confidence bar fills.
- AI Cluster Insights paragraph appears (may take 5–15s).
- Care Plan card appears below with medication reminders table and/or rawPlan text.

- [ ] **Step 4: Test patient history**

Change Patient ID to a value that has existing cluster history (e.g. try "P-001", "test-patient", or any ID you used in prior clustering runs). Expected: timeline entries appear below.

- [ ] **Step 5: Test PDF export**

Once the Care Plan is loaded, click "📄 Download PDF". Expected: browser prompts download of a file named `care-plan-P-001-<date>.pdf`.

- [ ] **Step 6: Test file upload**

Upload a screenshot or photo of a prescription image. Expected: upload zone shows "Extracting text…" then "✅ Text extracted and added to notes". The notes textarea should update with the extracted text.

- [ ] **Step 7: Final commit if any fixes were needed**

If any issues were found and fixed during smoke testing, commit:

```bash
git add medicluster/frontend/src/pages/ClinicalAIPage.jsx
git commit -m "fix: clinical AI smoke test corrections"
```
...
|---|---|
| Input zone — paste notes + file upload | Task 2 (notes textarea, file drop zone, aiChat) |
| Critical alerts — instant keyword scan | Task 2 (scanKeywords, CRITICAL_KEYWORDS array) |
| Critical alerts — NLP severity entities | Task 2 (nlpResult entities filter in Clinical Summary) |
| Clinical summary — entities table | Task 2 (entities table in nlpResult section) |
| Clinical summary — ICD-10 chips | Task 2 (icdCodes chip list) |
| Clinical summary — trajectory badge | Task 2 (trajectory conditional badge) |
| Clinical summary — AI summary blockquote | Task 2 (summary blockquote) |
| Drug interaction check | Task 2 (checkDrugInteractions after NLP) |
| Risk prediction — vitals form | Task 2 (6-field vitals grid) |
| Risk prediction — NLP auto-fill | Partially: vitals form pre-population not implemented (add as follow-up if desired; NLP entity shapes are unknown until tested) |
| Risk prediction — risk tier badge | Task 2 (conditional colour badge) |
| Risk prediction — confidence bar | Task 2 (LinearProgress-style div) |
| Risk prediction — AI insights | Task 2 (generateClusterInsights) |
| Care plan — medication table | Task 2 (reminders table) |
| Care plan — narrative text | Task 2 (rawPlan pre block) |
| Care plan — PDF export | Task 2 (exportToPdf) |
| Patient history timeline | Task 2 (vertical timeline with dots) |
| Route /clinical-ai | Task 1 |
| Nav link "Clinical AI" | Task 1 |
| No new packages | Confirmed — all deps already in package.json |
